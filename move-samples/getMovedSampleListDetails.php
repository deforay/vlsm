...
$general=new \Vlsm\Models\General($db);
        ...
        */
        ...
        $list = false;
        if(isset($_SESSION['privileges']) && (in_array("editSampleList.php", $_SESSION['privileges']))){
            $list = true;
        }
        ...
        foreach ($rResult as $aRow) {
            ...
            $row[] = $humanDate;
            if($list){
                $row[] = '<a href="editSampleList.php?id=' . base64_encode($aRow['move_sample_id']) . '" class="btn btn-primary btn-xs" style="margin-right: 2px;" title="Edit"><i class="fa fa-pencil"> Edit</i></a>&nbsp;';
            }else{
                $row[] = '';
            }
            $output['aaData'][] = $row;
