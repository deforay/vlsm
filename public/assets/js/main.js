...
function autoSelectSingleOption(selectedElementId) {
    let selectedElementOptions = $('#' + selectedElementId).find("option");
    if (selectedElementOptions !== null && selectedElementOptions.not(':empty').length === 1) {
        selectedElementOptions.each(function () {
            if ((this.value) != "") {
                $('#' + selectedElementId).val(this.value).trigger('change');
            }
        });
    }
}
