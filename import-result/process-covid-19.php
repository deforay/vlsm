...
try {
    ...
    if ($_POST['value'] != '') {
        for ($i = 0; $i < count($id); $i++) {
            ...
            if ($rResult[0]['sample_type'] != 'S' && $rResult[0]['sample_type'] != 's') {
                ...
                if ($status[$i] == '1') {
                    ...
                    $query = "select covid19_id, result from form_covid19 where sample_code='" . $sampleVal . "'";
        ...
        if (!file_exists(UPLOAD_PATH . DIRECTORY_SEPARATOR . "import-result" . DIRECTORY_SEPARATOR . $rResult[0]['import_machine_file_name'])) {
    ...
    $accQuery = "SELECT * FROM temp_sample_import as tsr LEFT JOIN form_covid19 as vl ON vl.sample_code=tsr.sample_code where imported_by ='$importedBy' AND tsr.result_status=7";
    ...
    if ($accResult) {
        for ($i = 0; $i < count($accResult); $i++) {
            ...
            if (!file_exists(UPLOAD_PATH . DIRECTORY_SEPARATOR . "import-result" . DIRECTORY_SEPARATOR . $accResult[$i]['import_machine_file_name'])) {
