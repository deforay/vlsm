...
if (isset($_SESSION['vlMonitoringResultQuery']) && trim($_SESSION['vlMonitoringResultQuery']) != "") {
    ...
    $sheet->setCellValue('A1', html_entity_decode('Viral Load Quarterly Monitoring Tool ', ENT_QUOTES, 'UTF-8'), \PhpOffice\PhpSpreadsheet\Cell\DataType::TYPE_STRING);
    $sheet->mergeCells('A3:M10');
    $sheet->setCellValue('A3', html_entity_decode($atomcolumns, ENT_QUOTES, 'UTF-8'), \PhpOffice\PhpSpreadsheet\Cell\DataType::TYPE_STRING);
    $sheet->mergeCells('A11:A12');
    $sheet->mergeCells('B11:F12');
    $sheet->setCellValue('B11', html_entity_decode('Question', ENT_QUOTES, 'UTF-8'), \PhpOffice\PhpSpreadsheet\Cell\DataType::TYPE_STRING);
    $sheet->mergeCells('G11:I12');
    $sheet->setCellValue('G11', html_entity_decode('Value', ENT_QUOTES, 'UTF-8'), \PhpOffice\PhpSpreadsheet\Cell\DataType::TYPE_STRING);
    $sheet->mergeCells('J11:M12');
    $sheet->setCellValue('J11', html_entity_decode('Comments', ENT_QUOTES, 'UTF-8'), \PhpOffice\PhpSpreadsheet\Cell\DataType::TYPE_STRING);
    ...
    $sheet->setCellValue('A13', html_entity_decode('Q1', ENT_QUOTES, 'UTF-8'), \PhpOffice\PhpSpreadsheet\Cell\DataType::TYPE_STRING);
    ...
    $sheet->setCellValue('A15', html_entity_decode('Q1.1', ENT_QUOTES, 'UTF-8'), \PhpOffice\PhpSpreadsheet\Cell\DataType::TYPE_STRING);
    $sheet->mergeCells('B15:F17');
    $sheet->setCellValue('B15', html_entity_decode('Of the number of Viral Load test results reported by lab,how many were: ', ENT_QUOTES, 'UTF-8'), \PhpOffice\PhpSpreadsheet\Cell\DataType::TYPE_STRING);
    ...
    $sheet->setCellValue('A18', html_entity_decode('Q1.2', ENT_QUOTES, 'UTF-8'), \PhpOffice\PhpSpreadsheet\Cell\DataType::TYPE_STRING);
    $sheet->mergeCells('B18:M18');
    $sheet->setCellValue('B18', html_entity_decode('Sex', ENT_QUOTES, 'UTF-8'), \PhpOffice\PhpSpreadsheet\Cell\DataType::TYPE_STRING);

    $sheet->mergeCells('B19:F19');
    $sheet->setCellValue('B19', html_entity_decode('Male', ENT_QUOTES, 'UTF-8'), \PhpOffice\PhpSpreadsheet\Cell\DataType::TYPE_STRING);
    $sheet->mergeCells('G19:G19');
    $sheet->setCellValue('G19', html_entity_decode($sResult[0]['ltMale1000'], ENT_QUOTES, 'UTF-8'), \PhpOffice\PhpSpreadsheet\Cell\DataType::TYPE_STRING);
    $sheet->setCellValue('H19', html_entity_decode($sResult[0]['gtMale1000'], ENT_QUOTES, 'UTF-8'), \PhpOffice\PhpSpreadsheet\Cell\DataType::TYPE_STRING);
    ...
    $sheet->setCellValue('A23', html_entity_decode('Q1.3', ENT_QUOTES, 'UTF-8'), \PhpOffice\PhpSpreadsheet\Cell\DataType::TYPE_STRING);
    $sheet->mergeCells('B23:M23');
    $sheet->setCellValue('B23', html_entity_decode('Age', ENT_QUOTES, 'UTF-8'), \PhpOffice\PhpSpreadsheet\Cell\DataType::TYPE_STRING);
    ...
    $sheet->getStyle('A35:M35')->getBorders()->getBottom()->setBorderStyle(\PhpOffice\PhpSpreadsheet\Style\Border::BORDER_THICK);
    //question two start
    $sheet->mergeCells('A36:A36');
    $sheet->setCellValue('A36', html_entity_decode('Q2', ENT_QUOTES, 'UTF-8'), \PhpOffice\PhpSpreadsheet\Cell\DataType::TYPE_STRING);
    ...
    if (count($checkEmptyResult) > 0) {
        foreach ($sampleTypeResult as $sampleName) {
            if ($checkEmptySampleResult[$sampleName['sample_name']][0]['total'] != 0) {
                ...
                $sheet->setCellValue('B' . $s, html_entity_decode('If yes,which type of samples?', ENT_QUOTES, 'UTF-8'), \PhpOffice\PhpSpreadsheet\Cell\DataType::TYPE_STRING);
                $sheet->mergeCells('G' . $s . ':I' . $ss);
                $sheet->mergeCells('J' . $s . ':M' . $ss);
                $sheet->setCellValue('G' . $s, html_entity_decode($sampleName['sample_name'], ENT_QUOTES, 'UTF-8'), \PhpOffice\PhpSpreadsheet\Cell\DataType::TYPE_STRING);
                $c = $s + 2;
                $cc = $ss + 1;
                $sheet->mergeCells('A' . $c . ':A' . $cc);
                $sheet->setCellValue('A' . $c, html_entity_decode('Q2.1', ENT_QUOTES, 'UTF-8'), \PhpOffice\PhpSpreadsheet\Cell\DataType::TYPE_STRING);
    ...
    if (isset($c) && isset($cc)) {
        $sheet->getStyle('A' . $c . ':M' . $cc)->getBorders()->getBottom()->setBorderStyle(\PhpOffice\PhpSpreadsheet\Style\Border::BORDER_THICK);
    } else {
        $sheet->getStyle('A37:M37')->getBorders()->getBottom()->setBorderStyle(\PhpOffice\PhpSpreadsheet\Style\Border::BORDER_THICK);
    }
    ...
    $sheet->setCellValue('A' . $q3, html_entity_decode('Q3', ENT_QUOTES, 'UTF-8'), \PhpOffice\PhpSpreadsheet\Cell\DataType::TYPE_STRING);
    ...
    $sheet->setCellValue('A' . $q4, html_entity_decode('Q4', ENT_QUOTES, 'UTF-8'), \PhpOffice\PhpSpreadsheet\Cell\DataType::TYPE_STRING);
    $sheet->mergeCells('B' . $q4 . ':F' . $q44);
    $sheet->setCellValue('B' . $q4, html_entity_decode('Are there planned procurements within this fiscal year? Choose from list', ENT_QUOTES, 'UTF-8'), \PhpOffice\PhpSpreadsheet\Cell\DataType::TYPE_STRING);
    ...
    $q44 = $ss + 4;
    $sheet->mergeCells('A' . $q4 . ':A' . $q44);
    $sheet->setCellValue('A' . $q4, html_entity_decode('Q4.1', ENT_QUOTES, 'UTF-8'), \PhpOffice\PhpSpreadsheet\Cell\DataType::TYPE_STRING);
    $sheet->mergeCells('B' . $q4 . ':F' . $q44);
    $sheet->setCellValue('B' . $q4, html_entity_decode('If yes, please list:', ENT_QUOTES, 'UTF-8'), \PhpOffice\PhpSpreadsheet\Cell\DataType::TYPE_STRING);
    $sheet->mergeCells('G' . $q4 . ':I' . $q44);
    $sheet->mergeCells('J' . $q4 . ':M' . $q44);
    $q4 = $q4 + 2;
    $q44 = $q44 + 2;
    $sheet->mergeCells('B' . $q4 . ':F' . $q44);
    $sheet->setCellValue('B' . $q4, html_entity_decode('Platform Type:', ENT_QUOTES, 'UTF-8'), \PhpOffice\PhpSpreadsheet\Cell\DataType::TYPE_STRING);
    $sheet->mergeCells('G' . $q4 . ':I' . $q44);
    $sheet->mergeCells('J' . $q4 . ':M' . $q44);
    $q4 = $q4 + 2;
    $q44 = $q44 + 2;
    $sheet->mergeCells('B' . $q4 . ':F' . $q44);
    $sheet->setCellValue('B' . $q4, html_entity_decode('Quantity:', ENT_QUOTES, 'UTF-8'), \PhpOffice\PhpSpreadsheet\Cell\DataType::TYPE_STRING);
    $sheet->mergeCells('G' . $q4 . ':I' . $q44);
    $sheet->mergeCells('J' . $q4 . ':M' . $q44);
    $q4 = $q4 + 2;
    $q44 = $q44 + 2;
    $sheet->mergeCells('B' . $q4 . ':F' . $q44);
    $sheet->setCellValue('B' . $q4, html_entity_decode('Planned location of placement:', ENT_QUOTES, 'UTF-8'), \PhpOffice\PhpSpreadsheet\Cell\DataType::TYPE_STRING);
    $sheet->mergeCells('G' . $q4 . ':I' . $q44);
    $sheet->mergeCells('J' . $q4 . ':M' . $q44);
    $sheet->getStyle('A' . $q4 . ':M' . $q44)->getBorders()->getBottom()->setBorderStyle(\PhpOffice\PhpSpreadsheet\Style\Border::BORDER_THICK);
    //question five start
    $q5 = $q4 + 2;
    $q55 = $q44 + 2;
    $sheet->mergeCells('A' . $q5 . ':A' . $q55);
    $sheet->setCellValue('A' . $q5, html_entity_decode('Q5', ENT_QUOTES, 'UTF-8'), \PhpOffice\PhpSpreadsheet\Cell\DataType::TYPE_STRING);
    $sheet->mergeCells('B' . $q5 . ':F' . $q55);
    $sheet->setCellValue('B' . $q5, html_entity_decode('Number of Early Infant Diagnosis tests reported by the lab:', ENT_QUOTES, 'UTF-8'), \PhpOffice\PhpSpreadsheet\Cell\DataType::TYPE_STRING);
    ...
    $q55 = $q55 + 2;
    $sheet->mergeCells('A' . $q5 . ':A' . $q55);
    $sheet->setCellValue('A' . $q5, html_entity_decode('Q5.1', ENT_QUOTES, 'UTF-8'), \PhpOffice\PhpSpreadsheet\Cell\DataType::TYPE_STRING);
    $sheet->mergeCells('B' . $q5 . ':F' . $q55);
    $sheet->setCellValue('B' . $q5, html_entity_decode('Number of Early Infant Diagnosis tests with a positive result:', ENT_QUOTES, 'UTF-8'), \PhpOffice\PhpSpreadsheet\Cell\DataType::TYPE_STRING);
    $sheet->mergeCells('G' . $q5 . ':I' . $q55);
    $sheet->mergeCells('J' . $q5 . ':M' . $q55);
    $sheet->getStyle('A' . $q5 . ':M' . $q55)->getBorders()->getBottom()->setBorderStyle(\PhpOffice\PhpSpreadsheet\Style\Border::BORDER_THICK);
    //question six start
    $q6 = $q5 + 2;
    $q66 = $q55 + 2;
    $sheet->mergeCells('A' . $q6 . ':A' . $q66);
    $sheet->setCellValue('A' . $q6, html_entity_decode('Q6', ENT_QUOTES, 'UTF-8'), \PhpOffice\PhpSpreadsheet\Cell\DataType::TYPE_STRING);
    $sheet->mergeCells('B' . $q6 . ':F' . $q66);
    $sheet->setCellValue('B' . $q6, html_entity_decode('Is there a backlog for Early Infant Diagnosis testing (greater than 1 month testing volume)? Choose from list', ENT_QUOTES, 'UTF-8'), \PhpOffice\PhpSpreadsheet\Cell\DataType::TYPE_STRING);
    $sheet->mergeCells('G' . $q6 . ':I' . $q66);
    $sheet->mergeCells('J' . $q6 . ':M' . $q66);
    $sheet->setCellValue('J' . $q6, html_entity_decode('Reasons:', ENT_QUOTES, 'UTF-8'), \PhpOffice\PhpSpreadsheet\Cell\DataType::TYPE_STRING);

    $q6 = $q6 + 2;
    $q66 = $q66 + 2;
    $sheet->mergeCells('A' . $q6 . ':A' . $q66);
    $sheet->setCellValue('A' . $q6, html_entity_decode('Q6.1', ENT_QUOTES, 'UTF-8'), \PhpOffice\PhpSpreadsheet\Cell\DataType::TYPE_STRING);
    $sheet->mergeCells('B' . $q6 . ':F' . $q66);
    $sheet->setCellValue('B' . $q6, html_entity_decode('If yes, how many samples?', ENT_QUOTES, 'UTF-8'), \PhpOffice\PhpSpreadsheet\Cell\DataType::TYPE_STRING);
    $sheet->mergeCells('G' . $q6 . ':I' . $q66);
    $sheet->mergeCells('J' . $q6 . ':M' . $q66);
    $sheet->getStyle('A' . $q6 . ':M' . $q66)->getBorders()->getBottom()->setBorderStyle(\PhpOffice\PhpSpreadsheet\Style\Border::BORDER_THICK);
    ...
    $sheet->setCellValue('A' . $q7, html_entity_decode('Q7', ENT_QUOTES, 'UTF-8'), \PhpOffice\PhpSpreadsheet\Cell\DataType::TYPE_STRING);
    $sheet->mergeCells('B' . $q7 . ':F' . $q77);
    $sheet->setCellValue('B' . $q7, html_entity_decode('Number of invalid VL and EID tests reported by the lab per month:', ENT_QUOTES, 'UTF-8'), \PhpOffice\PhpSpreadsheet\Cell\DataType::TYPE_STRING);
    ...
    $sheet->getStyle('A' . $bQ7 . ':M' . $bQ7)->getBorders()->getBottom()->setBorderStyle(\PhpOffice\PhpSpreadsheet\Style\Border::BORDER_THICK);
    //question eight start
    $q8 = $q7;
    $q88 = $q77;
    $mergeQ8 = $q8;
    $sheet->mergeCells('A' . $q8 . ':A' . $q88);
    $sheet->setCellValue('A' . $q8, html_entity_decode('Q8', ENT_QUOTES, 'UTF-8'), \PhpOffice\PhpSpreadsheet\Cell\DataType::TYPE_STRING);
    $sheet->mergeCells('B' . $q8 . ':F' . $q88);
    $sheet->setCellValue('B' . $q8, html_entity_decode('Duration of equipment breakdown in number of days (provide per instrument type):', ENT_QUOTES, 'UTF-8'), \PhpOffice\PhpSpreadsheet\Cell\DataType::TYPE_STRING);
    $sheet->setCellValue('G' . $q8, html_entity_decode('Instrument', ENT_QUOTES, 'UTF-8'), \PhpOffice\PhpSpreadsheet\Cell\DataType::TYPE_STRING);
    $sheet->mergeCells('H' . $q8 . ':I' . $q8);
    $sheet->mergeCells('J' . $q8 . ':M' . $q8);
    $sheet->getStyle('G' . $q8 . ':M' . $q8)->applyFromArray($questionStyle);
    $sheet->setCellValue('H' . $q8, html_entity_decode('Days Down', ENT_QUOTES, 'UTF-8'), \PhpOffice\PhpSpreadsheet\Cell\DataType::TYPE_STRING);
    $q8 = $q8 + 1;
    $q88 = $q88 + 1;
    $sheet->mergeCells('H' . $q8 . ':I' . $q8);
    $sheet->mergeCells('J' . $q8 . ':M' . $q8);
    ...
    $sheet->mergeCells('A' . $q8 . ':A' . $q88);
    $sheet->setCellValue('A' . $q8, html_entity_decode('Q8.1', ENT_QUOTES, 'UTF-8'), \PhpOffice\PhpSpreadsheet\Cell\DataType::TYPE_STRING);
    $sheet->mergeCells('B' . $q8 . ':F' . $q88);
    $sheet->setCellValue('B' . $q8, html_entity_decode('Reason for instrument breakdown: ', ENT_QUOTES, 'UTF-8'), \PhpOffice\PhpSpreadsheet\Cell\DataType::TYPE_STRING);
    $sheet->mergeCells('G' . $q8 . ':I' . $q88);
    $sheet->mergeCells('J' . $q8 . ':M' . $q88);
    $sheet->getStyle('A37:F' . $q8)->applyFromArray($questionStyle);
    $sheet->getStyle('A' . $mergeQ8 . ':F' . $mergeQ8)->getAlignment()->setVertical(\PhpOffice\PhpSpreadsheet\Style\Alignment::VERTICAL_CENTER);
    $sheet->getStyle('A' . $mergeQ7 . ':F' . $mergeQ7)->getAlignment()->setVertical(\PhpOffice\PhpSpreadsheet\Style\Alignment::VERTICAL_CENTER);
