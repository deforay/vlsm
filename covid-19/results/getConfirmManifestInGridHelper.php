...
$sQuery = "SELECT * FROM form_covid19 as vl    
                    ...
                    INNER JOIN covid19_positive_confirmation_manifest as pcm ON pcm.manifest_id=vl.positive_test_manifest_id 
...
$sWhere = $sWhere . 'AND vl.positive_test_manifest_id IS NOT NULL AND vl.positive_test_manifest_id != ""';
/* if ($sWhere != '') {
...
$filterQuery = "SELECT * FROM form_covid19 as vl    
LEFT JOIN facility_details as f ON vl.facility_id=f.facility_id 
INNER JOIN r_sample_status as ts ON ts.status_id=vl.result_status 
INNER JOIN covid19_positive_confirmation_manifest as pcm ON pcm.manifest_id=vl.positive_test_manifest_id 
LEFT JOIN batch_details as b ON b.batch_id=vl.sample_batch_id $sWhere";
$aResultFilterTotal = $db->rawQuery($filterQuery);
$iFilteredTotal = count($aResultFilterTotal);

/* Total data set length */
$TotalQuery = "SELECT COUNT(covid19_id) as total FROM form_covid19 as vl    
LEFT JOIN facility_details as f ON vl.facility_id=f.facility_id 
INNER JOIN r_sample_status as ts ON ts.status_id=vl.result_status 
INNER JOIN covid19_positive_confirmation_manifest as pcm ON pcm.manifest_id=vl.positive_test_manifest_id 
LEFT JOIN batch_details as b ON b.batch_id=vl.sample_batch_id $sWhere";

$aResultTotal =  $db->rawQuery($TotalQuery);
...
foreach ($rResult as $aRow) {
     ...
     $row[] = $aRow['positive_test_manifest_code'];
