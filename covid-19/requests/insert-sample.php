...
if (isset($_POST['provinceId']) && $_POST['provinceId'] != null && $_POST['provinceId'] != '') {
    if (isset($existResult[0][$sampleCodeKey]) && $existResult[0][$sampleCodeKey] != '') {
        ...
        $svlQuery = 'SELECT ' . $sampleCodeKey . ' FROM form_covid19 as vl WHERE DATE(vl.sample_collection_date) >= "' . $start_date . '" AND DATE(vl.sample_collection_date) <= "' . $end_date . '" AND province_id=' . $_POST['provinceId'] . ' AND ' . $sampleCode . ' IS NOT NULL AND ' . $sampleCode . '!= "" ORDER BY ' . $sampleCodeKey . ' DESC LIMIT 1';
        ...
        $sCode = $rKey . "R" . date('y') . $_POST['provinceCode'] . "C19" . $maxId;
        $j = 1;
        do {
            $sQuery = "SELECT sample_code FROM form_covid19 as vl where sample_code='" . $sCode . "'";
