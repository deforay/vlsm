...
$aResultTotal =  $db->rawQuery("select COUNT(result_id) as total FROM r_covid19_results");
...
foreach ($rResult as $aRow) {
    ...
    if (isset($_SESSION['privileges']) && in_array("covid19-sample-type.php", $_SESSION['privileges']) && $sarr['sc_user_type'] !='vluser') {
        $row[] = '<a href="edit-covid19-results.php?id=' . base64_encode($aRow['result_id']) . '" class="btn btn-primary btn-xs" style="margin-right: 2px;" title="Edit"><i class="fa fa-pencil"> Edit</i></a>';
    }
    $output['aaData'][] = $row;
}
