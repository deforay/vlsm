...
if(isset($_POST['toEmail']) && trim($_POST['toEmail'])!= '' && count($_POST['sample']) >0){
     ...
     if(isset($mailconf['rq_field']) && trim($mailconf['rq_field'])!= ''){
         ...
         for($s=0;$s<count($_POST['sample']);$s++){
          ...
          $filePlace = $pathFront. DIRECTORY_SEPARATOR . $filename;
     }else{
          $_SESSION['alertMsg']='Unable to generate the excel file. Please check the request fields.';
          header('location:vlRequestMail.php');
     }
}else{
     $_SESSION['alertMsg']='Unable to generate the excel file. Please try later.';
     header('location:vlRequestMail.php');
}
?>
                    ...
                    <form id="vlRequestMailConfirmForm" name="vlRequestMailConfirmForm" method="post" action="vlRequestMailHelper.php">
                        <div class="row">
                            ...
                            <div class="col-lg-12" style="text-align:center;padding-left:0;">
                                <h1>Are you sure you want to send this mail..?</h1>
                                <p>To send, Please click 'YES' button</p>
                                <a href="../mail/vlRequestMail.php" class="btn btn-default"> NO</a>&nbsp;
                                <a class="btn btn-primary" href="javascript:void(0);" onclick="confirmRequestMail();">YES</a>
                                <p style="margin-top:10px;">OR &nbsp;<a href="<?php echo $filePlace; ?>" style="text-decoration:none;">Click here to download the excel</a></p>
                            </div>
                        </div>
                    </form>
                </div>
