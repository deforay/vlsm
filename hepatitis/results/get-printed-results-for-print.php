...
$aResultFilterTotal = $db->rawQuery("SELECT hepatitis_id FROM form_hepatitis as vl 
					LEFT JOIN facility_details as f ON vl.facility_id=f.facility_id 
...
$aResultTotal =  $db->rawQuery("select COUNT(hepatitis_id) as total FROM form_hepatitis as vl $dWhere");
...
foreach ($rResult as $aRow) {
	...
	if (isset($_POST['vlPrint']) && $_POST['vlPrint'] == 'print') {
		...
		$print = '<a href="javascript:void(0);" class="btn btn-primary btn-xs" style="margin-right: 2px;" title="'. _("Print").'" onclick="resultPDF(' . $aRow['hepatitis_id'] . ',\'printData\');"><i class="fa fa-print"> '. _("Print").'</i></a>';
	}
