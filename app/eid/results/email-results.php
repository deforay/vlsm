...
$title = _translate("Email EID Test Results");
...
$sTypeQuery = "SELECT * FROM r_eid_sample_type WHERE `status`='active'";
$sTypeResult = $db->rawQuery($sTypeQuery);

$pdQuery = "SELECT * FROM geographical_divisions WHERE geo_parent = 0 and geo_status='active'";
$pdResult = $db->query($pdQuery);
$batchQuery = "SELECT * FROM batch_details WHERE test_type='eid' AND batch_status='completed'";
...
<script type="text/javascript">
	...
	function convertSearchResultToPdf() {
		...
		$.post("/eid/results/generate-result-pdf.php", {
				source: 'print',
				id: id,
