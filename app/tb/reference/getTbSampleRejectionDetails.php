...
$aResultFilterTotal = $db->rawQuery("SELECT * FROM r_tb_sample_rejection_reasons $sWhere order by $sOrder");
$iFilteredTotal = count($aResultFilterTotal);

/* Total data set length */
$aResultTotal = $db->rawQuery("select COUNT(rejection_reason_id) as total FROM r_tb_sample_rejection_reasons");
...
foreach ($rResult as $aRow) {
    ...
    if (!empty($_SESSION['privileges']) && array_key_exists("tb-sample-type.php", $_SESSION['privileges']) && $sarr['sc_user_type'] != 'vluser') {
        $row[] = $status;
    } else {
