   ...
   if (isset($mailconf['rs_field']) && trim($mailconf['rs_field']) != '') {
      ...
      class MYPDF extends TCPDF
      {
         ...
         public function Header()
         {
            ...
            $this->writeHTMLCell(0, 0, 10, 18, 'HEPATITIS TEST RESULT', 0, 0, 0, true, 'C', true);
      ...
      for ($s = 0; $s < count($_POST['sample']); $s++) {
         $sampleQuery = "SELECT sample_code FROM form_hepatitis as vl LEFT JOIN facility_details as f ON vl.facility_id=f.facility_id where vl.hepatitis_id = '" . $_POST['sample'][$s] . "' AND vl.result IS NOT NULL AND vl.result!= '' ORDER BY f.facility_name ASC";
         ...
         if (isset($sampleResult[0]['sample_code'])) {
            ...
            for ($f = 0; $f < count($filedGroup); $f++) {
               ...
               if ($filedGroup[$f] == "Sample ID") {
                  ...
                  $field = 'reason_for_hepatitis_test';
               ...
               if ($field ==  'result_reviewed_by') {
                  $fValueQuery = "SELECT u.user_name as reviewedBy FROM form_hepatitis as vl LEFT JOIN facility_details as f ON vl.facility_id=f.facility_id LEFT JOIN r_hepatitis_sample_type as s_type ON s_type.sample_id=vl.sample_type LEFT JOIN r_hepatitis_sample_rejection_reasons as s_r_r ON s_r_r.rejection_reason_id=vl.reason_for_sample_rejection LEFT JOIN user_details as u ON u.user_id = vl.result_reviewed_by where vl.hepatitis_id = '" . $_POST['sample'][$s] . "'";
               } elseif ($field ==  'result_approved_by') {
                  $fValueQuery = "SELECT u.user_name as approvedBy FROM form_hepatitis as vl LEFT JOIN facility_details as f ON vl.facility_id=f.facility_id LEFT JOIN r_hepatitis_sample_type as s_type ON s_type.sample_id=vl.sample_type LEFT JOIN r_hepatitis_sample_rejection_reasons as s_r_r ON s_r_r.rejection_reason_id=vl.reason_for_sample_rejection LEFT JOIN user_details as u ON u.user_id = vl.result_approved_by where vl.hepatitis_id = '" . $_POST['sample'][$s] . "'";
               } elseif ($field ==  'lab_id') {
                  $fValueQuery = "SELECT f.facility_name as labName FROM form_hepatitis as vl LEFT JOIN facility_details as f ON vl.lab_id=f.facility_id where vl.hepatitis_id = '" . $_POST['sample'][$s] . "'";
               } else {
                  $fValueQuery = "SELECT $field FROM form_hepatitis as vl LEFT JOIN facility_details as f ON vl.facility_id=f.facility_id LEFT JOIN r_hepatitis_sample_type as s_type ON s_type.sample_id=vl.sample_type LEFT JOIN r_hepatitis_sample_rejection_reasons as s_r_r ON s_r_r.rejection_reason_id=vl.reason_for_sample_rejection LEFT JOIN r_sample_status as t_s ON t_s.status_id=vl.result_status where vl.hepatitis_id = '" . $_POST['sample'][$s] . "'";
               }
               ...
               if (count($fValueResult) > 0) {
                  if ($field == 'sample_collection_date' || $field == 'sample_received_at_vl_lab_datetime' || $field == 'sample_tested_datetime') {
                     if (isset($fValueResult[0][$field]) && trim($fValueResult[0][$field]) != '' && trim($fValueResult[0][$field]) != '0000-00-00 00:00:00') {
                        ...
                        $fieldValue = \App\Utilities\DateUtils::humanReadableDateFormat($xplodDate[0]) . " " . $xplodDate[1];
                     }
