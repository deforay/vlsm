...
/*
         ...
         * Get data to display
        */
$sQuery = "SELECT SQL_CALC_FOUND_ROWS vl.*,f.*,s.*,fd.facility_name as labName,ts.status_name FROM form_vl as vl LEFT JOIN facility_details as f ON vl.facility_id=f.facility_id LEFT JOIN facility_details as fd ON fd.facility_id=vl.lab_id LEFT JOIN r_vl_sample_type as s ON s.sample_id=vl.sample_type LEFT JOIN batch_details as b ON b.batch_id=vl.sample_batch_id LEFT JOIN r_vl_art_regimen as art ON vl.current_regimen=art.art_id 
...
// $aResultFilterTotal = $db->rawQuery("SELECT vl.*,f.*,s.*,fd.facility_name as labName FROM form_vl as vl LEFT JOIN facility_details as f ON vl.facility_id=f.facility_id LEFT JOIN facility_details as fd ON fd.facility_id=vl.lab_id LEFT JOIN r_vl_sample_type as s ON s.sample_id=vl.sample_type LEFT JOIN batch_details as b ON b.batch_id=vl.sample_batch_id LEFT JOIN r_vl_art_regimen as art ON vl.current_regimen=art.art_id where vl.result_status!=4 AND vl.sample_code is NOT NULL AND  (vl.result IS NULL OR vl.result='') $sWhere group by vl.vl_sample_id order by $sOrder");
// $iFilteredTotal = count($aResultFilterTotal);

// /* Total data set length */
// $aResultTotal =  $db->rawQuery("select COUNT(vl_sample_id) as total FROM form_vl as vl where result_status!=4 AND  vl.sample_code is NOT NULL AND (vl.result IS NULL OR vl.result='') AND vlsm_country_id='" . $arr['vl_form'] . "' $dWhere");
