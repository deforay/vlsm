...
$sQuery = "SELECT SQL_CALC_FOUND_ROWS  SUM(CASE WHEN vl.sample_tested_datetime is not null THEN 1 ELSE 0 END) as `testcount`, 
                ...
                FROM form_generic vl, batch_details b ";
...
foreach ($rResult as $aRow) {
    ...
    $row = [];
    $printBarcode = '<a href="/generic-tests/requests/batch/generateBarcode.php?id=' . base64_encode($aRow['batch_id']) . '&code=' . $_POST['type'] . '" target="_blank"  rel="noopener" class="btn btn-info btn-xs" style="margin-right: 2px;" title="' . _("Print bar code") . '"><em class="fa-solid fa-barcode"></em> ' . _("Print Batch PDF") . '</a>';
