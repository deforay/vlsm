...
$bQuery = "SELECT vl.sample_code,vl.sample_batch_id,vl.sample_id,vl.facility_id,vl.result,vl.result_status,f.facility_name,f.facility_code FROM form_generic as vl INNER JOIN facility_details as f ON vl.facility_id=f.facility_id WHERE  (vl.is_sample_rejected IS NULL OR vl.is_sample_rejected = '' OR vl.is_sample_rejected = 'no') AND (vl.reason_for_sample_rejection IS NULL OR vl.reason_for_sample_rejection ='' OR vl.reason_for_sample_rejection = 0) AND vl.sample_code!='' AND vl.sample_batch_id = $id ORDER BY vl.last_modified_datetime ASC";

//echo '<pre>'; print_r($batchInfo); die;
$batchResultresult = $db->rawQuery($bQuery);
$query = "SELECT vl.sample_code,vl.sample_batch_id,vl.sample_id,vl.facility_id,vl.result,vl.result_status,f.facility_name,f.facility_code FROM form_generic as vl INNER JOIN facility_details as f ON vl.facility_id=f.facility_id WHERE (vl.sample_batch_id IS NULL OR vl.sample_batch_id = '') AND (vl.is_sample_rejected IS NULL OR vl.is_sample_rejected = '' OR vl.is_sample_rejected = 'no') AND (vl.reason_for_sample_rejection IS NULL OR vl.reason_for_sample_rejection ='' OR vl.reason_for_sample_rejection = 0) AND (vl.result is NULL or vl.result = '') AND vl.sample_code!='' ORDER BY vl.last_modified_datetime ASC";
	...
	<section class="content">
		...
		<div class="box box-default">
			...
			<table aria-describedby="table" class="table" aria-hidden="true" style="margin-top:20px;width: 100%;">
				...
				<tr>
					<td style="width: 20%;">&nbsp;<strong>Facility Name & Code&nbsp;:</strong></td>
					<td style="width: 30%;">
				...
				<tr class="showFemaleSection">
					<td style="width: 20%;"><strong>Pregnant&nbsp;:</strong></td>
					<td style="width: 30%;">
					...
					<td style="width: 20%;"><strong>Is Patient Breastfeeding&nbsp;:</strong></td>
					<td style="width: 30%;">
...
<script type="text/javascript">
	...
	$(document).ready(function() {
		...
		?>
                    ...
                    <?php } ?>
                    resultSampleArray.push('<?php echo $sample['sample_id']; ?>');
        <?php $r++;
				}
			}
		}
