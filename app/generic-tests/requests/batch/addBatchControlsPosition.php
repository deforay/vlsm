...
if (isset($prevlabelInfo[0]['label_order']) && trim($prevlabelInfo[0]['label_order']) != '') {
		...
		if (count($xplodJsonToArray) > 1 && $xplodJsonToArray[0] == "s") {
			if (isset($displaySampleOrderArray[$sCount]) && $displaySampleOrderArray[$sCount] > 0) {
				if ($sCount <= $prevDisplaySampleArray) {
					...
					$sampleQuery = "SELECT sample_code from form_generic where sample_id=$displaySampleOrderArray[$sCount]";
	...
	for ($ns = 0; $ns < count($remainSampleNewArray); $ns++) {
		...
		$sampleQuery = "SELECT sample_code from form_generic where sample_id=$remainSampleNewArray[$ns]";
		$sampleResult = $db->query($sampleQuery);
		$label = $sampleResult[0]['sample_code'];
...
<div class="content-wrapper">
	...
	<section class="content">
		<div class="box box-default">
			...
			<div class="box-body">
				<!-- form start -->
				<form class="form-horizontal" method='post' name='addBatchControlsPosition' id='addBatchControlsPosition' autocomplete="off" action="/generic-tests/requests/batch/addBatchControlsPositionHelper.php">
					<div class="box-body">
