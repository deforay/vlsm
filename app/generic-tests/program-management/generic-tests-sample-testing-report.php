...
try {
    ...
    if ($_SESSION['instance']['type'] != 'remoteuser') {
        ...
        if (!empty($_SESSION['facilityMap'])) {
            $whereCondition = " AND generic.facility_id IN (" . $_SESSION['facilityMap'] . ") ";
        }
    }
    ...
    $sQuery = "SELECT
                ...
                FROM form_generic as generic JOIN facility_details as f ON f.facility_id=generic.facility_id
                WHERE DATE(generic.sample_collection_date) BETWEEN '$startDate' AND '$endDate'
                $whereCondition
                GROUP BY generic.facility_id ORDER BY totalCount DESC";
