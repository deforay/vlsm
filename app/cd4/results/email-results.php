...
$title = _translate("Email CD4 Test Results");
...
$sTypeQuery = "SELECT * FROM r_cd4_sample_types WHERE `status`='active'";
$sTypeResult = $db->rawQuery($sTypeQuery);

$pdQuery = "SELECT * FROM geographical_divisions WHERE geo_parent = 0 and geo_status='active'";
$pdResult = $db->query($pdQuery);
$batchQuery = "SELECT * FROM batch_details WHERE test_type='cd4' AND batch_status='completed'";
						...
						<div class="row">
							<div class="col-md-9">
								<div class="form-group">
									...
									<div class="col-lg-9">
										<input type="text" id="subject" name="subject" class="form-control isRequired" placeholder="<?php echo _translate('Subject'); ?>" title="<?php echo _translate('Please enter subject'); ?>" value="CD4 Test Results" />
									</div>
								</div>
							</div>
						</div>
...
<script type="text/javascript">
	...
	function getSampleDetails() {
		...
		var type = $('#type').val();
		$.post("/cd4/results/getRequestSampleCodeDetails.php", {
	...
	function convertSearchResultToPdf() {
		...
		$.post("/cd4/results/generate-result-pdf.php", {
				source: 'print',
				id: id,
