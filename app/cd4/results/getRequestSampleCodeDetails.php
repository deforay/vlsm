...
$query = "SELECT vl.sample_code,vl.cd4_id,vl.facility_id,f.facility_name,f.facility_code
        FROM form_cd4 as vl LEFT JOIN facility_details as f ON vl.facility_id=f.facility_id
...
if (trim((string) $mailSentStatus) != '') {
  if (trim((string) $type) == 'request') {
    ...
    $query = $query . " AND vl.is_result_mail_sent='" . $mailSentStatus . "' AND ((vl.result_status = 7 AND vl.cd4_result is NOT NULL AND vl.cd4_result !='') OR (vl.result_status = 4 AND (vl.cd4_result is NULL OR vl.cd4_result = '')))";
  }
}
  ...
  <div class="form-group">
    ...
    <div class="col-lg-9">
      ...
      <select id="sample" name="sample[]" multiple="multiple" class="search isRequired" title="<?php echo _translate('Please select sample(s)'); ?>">
        ...
        ?>
            <option value="<?php echo $sample['cd4_id']; ?>"><?= ($sample['sample_code']) . " - " . ($sample['facility_name']); ?></option>
        <?php
          }
        }
        ?>
