            ...
            if ($rResult[0]['sample_type'] != 'S' && $rResult[0]['sample_type'] != 's') {
                ...
                if ($status[$i] == '1') {
                    ...
                    if ($bvlResult) {
                        ...
                        $batchResult = $db->insert('batch_details', array('test_type' => 'eid', 'batch_code' => $rResult[0]['batch_code'], 'batch_code_key' => $rResult[0]['batch_code_key'], 'sent_mail' => 'no', 'request_created_datetime' => $db->now()));
