...
try {
	if (isset($_POST['provinceName']) && trim($_POST['provinceName']) != "") {
		...
		if (isset($_POST['provinceId']) && $_POST['provinceId'] != "") {
			...
			$facilityQuery = "SELECT facility_id, facility_state from $facilityTable where facility_state=? LIMIT 1";
			$facilityInfo = $db->rawQuery($facilityQuery, [$_POST['provinceNameOld']]);
