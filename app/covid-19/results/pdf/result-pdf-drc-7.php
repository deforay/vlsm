    ...
    class DRCCovid19PDF7 extends Covid19ResultPDFHelper
    {
        ...
        public function Header()
        {
            ...
            if (!empty($this->htitle) && trim($this->htitle) != '') {
                ...
                $this->writeHTMLCell(0, 0, 0, 32, '<em>Service de Biologie Moléculaire</em>', 0, 0, 0, true, 'C');
                $this->SetTextColor(0, 0, 0);
                $this->SetFont('helvetica', 'U', 11);
                $this->writeHTMLCell(0, 0, 0, 38, 'Laboratoire National de Référence pour la Grippe et les Virus Respiratoires', 0, 0, 0, true, 'C');

                $this->SetFont('helvetica', 'B', 10);
                $this->writeHTMLCell(0, 0, 0, 44, 'RÉSULTATS DES LABORATOIRES DES ECHANTILLONS RESPIRATOIRES', 0, 0, 0, true, 'C');
                $this->SetFont('helvetica', 'U', 10);
                $this->writeHTMLCell(0, 0, 0, 49, 'TEST AU nCOV-19 PAR RT-PCR en temps réel', 0, 0, 0, true, 'C');

                $this->writeHTMLCell(0, 0, 10, 54, '<hr>', 0, 0, 0, true, 'C');
...
$html .= '<td width="100%" style="line-height:10px;font-size:11px;text-align:center;" colspan="3">
            <br><br><strong>Fait à Lubumbashi, le: </strong>' . DateUtility::humanReadableDateFormat($result['result_approved_datetime']) .
...
$labManager = "";
if (!empty($result['lab_manager'])) {
    $labManagerRes = $usersService->getUserByID($result['lab_manager'], array('user_name', 'user_signature'));
    if ($labManagerRes) {
    ...
    if (!empty($labManagerRes['user_signature'])) {
        ...
        if (MiscUtility::isImageValid($labManagerSignaturePath)) {
