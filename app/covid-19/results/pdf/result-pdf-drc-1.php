...
if (!class_exists('DRCCovid19PDF1')) {
    ...
    class DRCCovid19PDF1 extends Covid19ResultPDFHelper
    {
        //Page header
        public function Header()
        {
            ...
            if (!empty($this->htitle) && trim($this->htitle) != '') {
                ...
                if (MiscUtility::isImageValid($imageFilePath)) {
                ...
                $this->writeHTMLCell(0, 0, 0, 22, '<em>Département de Virologie</em>', 0, 0, 0, true, 'C');
                $this->SetTextColor(0, 0, 0);
                $this->SetFont('helvetica', 'U', 11);
                $this->writeHTMLCell(0, 0, 0, 27, 'Laboratoire National de Référence Pour la Grippe et les virus respiratoires', 0, 0, 0, true, 'C');

                $this->SetFont('helvetica', 'B', 10);
                $this->writeHTMLCell(0, 0, 0, 36, 'RÉSULTATS DES LABORATOIRES DES ECHANTILLONS RESPIRATOIRES', 0, 0, 0, true, 'C');
                $this->SetFont('helvetica', 'U', 10);
                $this->writeHTMLCell(0, 0, 0, 42, 'TESTES AU nCOV-19 PAR RT-PCR en temps réel', 0, 0, 0, true, 'C');

                $this->writeHTMLCell(0, 0, 10, 48, '<hr>', 0, 0, 0, true, 'C');
                ...
                if (!empty($this->logo) && MiscUtility::isImageValid($img_file)) {
                ...
                // Render the image
                if (MiscUtility::isImageValid($img_file)) {
                    $this->SetAlpha(0.1);
...
$html .= '<tr>';
$html .= '<td colspan="3" style="line-height:12px;font-size:12px;text-align:center;"><br><br><strong>' . $labManager . '</strong><br>Chef de l&lsquo;unité Virus Respiratories<br><span style="font-size:8;font-weight:normal;">(Lab Manager)</span></td>';
$html .= '</tr>';
