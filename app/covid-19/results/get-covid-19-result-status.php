...
if ($_SESSION['instanceType'] == 'remoteuser') {
    ...
    $aColumns = array('vl.sample_code', "DATE_FORMAT(vl.sample_collection_date,'%d-%b-%Y')", 'b.batch_code', 'vl.patient_id', 'CONCAT(COALESCE(vl.patient_name,""), COALESCE(vl.patient_surname,""))', 'f.facility_name', 'f.facility_code',  'vl.result', "DATE_FORMAT(vl.last_modified_datetime,'%d-%b-%Y')", 'ts.status_name');
...
if (isset($sWhere) && count($sWhere) > 0) {
    ...
    if (isset($_POST['statusFilter']) && trim($_POST['statusFilter']) != '') {
        if (isset($setWhr)) {
            ...
            if ($_POST['statusFilter'] == 'approvedOrRejected') {
                $sWhere[] =  ' where vl.result_status IN (4,7)';
            } else if ($_POST['statusFilter'] == 'notApprovedOrRejected') {
                $sWhere[] = ' where vl.result_status NOT IN (4,7)';
            }
        }
    }
    if (isset($_POST['statusFilter']) && $_POST['statusFilter'] != '') {
...
foreach ($rResult as $aRow) {
    ...
    $row[] = $status;
    //$row[] = '<a href="updateVlTestResult.php?id=' . base64_encode($aRow['covid19_id']) . '" class="btn btn-success btn-xs" style="margin-right: 2px;" title="Result"><i class="fa-solid fa-pen-to-square"></i> Result</a>';
