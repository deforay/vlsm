...
try {
	...
	$covid19Data = array(
		...
		'health_outcome_date'                 => !empty($_POST['outcomeDate']) ? $general->isoDateFormat($_POST['outcomeDate']) : null,
		...
		'authorized_on' 					  => !empty($_POST['authorizedOn']) ? $general->isoDateFormat($_POST['authorizedOn']) : null,
			...
			foreach ($_POST['testName'] as $testKey => $testKitName) {
				if (isset($testKitName) && !empty($testKitName)) {
					...
					$covid19TestData = array(
						...
						'kit_expiry_date'      		=> (strpos($testKitName, 'RDT') !== false) ? $general->isoDateFormat($_POST['expDate'][$testKey]) : null,
	...
	if (!empty($_POST['api']) && $_POST['api'] == "yes") {
		...
		if ($id > 0) {
			...
			$action = ucwords($_SESSION['userName']) . ' added a new Covid-19 request with the Sample Code/ID ' . $_POST['sampleCode'] . ' (' . $_POST['covid19SampleId'] . ')';
