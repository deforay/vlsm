	...
	<?php if (!$hidesrcofreq) { ?>
		...
		<section class="content-header">
			<h1><em class="fa-solid fa-pen-to-square"></em>
				<?php echo _("Covid-19 Test Requests"); ?>
			</h1>
			<ol class="breadcrumb">
						...
						<tr>
							...
							<td>
								<input type="text" id="patientId" name="patientId" class="form-control"
									placeholder="<?php echo _('Patient ID'); ?>"
									title="<?php echo _('Please enter the patient ID to search'); ?>" />
							</td>
							...
							<td colspan="4">
								<?php if (isset($_SESSION['privileges']) && in_array("/covid-19/requests/covid-19-add-request.php", $_SESSION['privileges']) && !$hidesrcofreq) { ?>
									<a style=" margin: 0px 5px; " href="/covid-19/requests/covid-19-add-request.php"
										...
										<?php echo _("Add new Covid-19 Request"); ?>
									</a>
									<?php if ($global['vl_form'] == 1 && $_SESSION['instanceType'] != 'remoteuser' && !$hidesrcofreq) { ?>
										<a style=" margin: 0px 5px; " href="/covid-19/requests/covid-19-bulk-import-request.php"
											class="btn btn-primary btn-sm pull-right"> <em class="fa-solid fa-plus"></em>
											<?php echo _("Bulk Import Covid-19 Request"); ?>
										</a>
									<?php }
									if ($formId == 1 && $_SESSION['instanceType'] != 'remoteuser' && !$hidesrcofreq) { ?>
										<a style=" margin: 0px 5px; " href="/covid-19/requests/covid-19-quick-add.php"
											class="btn btn-primary btn-sm pull-right"> <em class="fa-solid fa-plus"></em>
											<?php echo _("Quick Add Covid-19 Request"); ?>
										</a>
									<?php }
								}
							...
							<td>
								...
								if (isset($_SESSION['privileges']) && in_array("/covid-19/requests/covid-19-add-request.php", $_SESSION['privileges']) && !$hidesrcofreq) { ?>
									<?php if ($formId == 1 && $_SESSION['instanceType'] != 'remoteuser') { ?>
										<a style=" margin: 0px 5px; " href="/covid-19/requests/covid-19-quick-add.php"
											class="btn btn-primary btn-sm pull-right"> <em class="fa-solid fa-plus"></em>
											<?= _("Quick Add Covid-19 Request"); ?>
										</a>
									<?php } ?>

									<a style=" margin: 0px 5px; " href="/covid-19/requests/covid-19-add-request.php"
										class="btn btn-primary btn-sm pull-right"> <em class="fa-solid fa-plus"></em>
										<?= _("Add new Covid-19 Request"); ?>
									</a>
									<?php if ($global['vl_form'] == 1 && $_SESSION['instanceType'] != 'remoteuser') { ?>
										<a style=" margin: 0px 5px; " href="/covid-19/requests/covid-19-bulk-import-request.php"
											class="btn btn-primary btn-sm pull-right"> <em class="fa-solid fa-plus"></em>
											<?= _("Bulk Import Covid-19 Requests"); ?>
										</a>
									<?php }
								}
								if (isset($_SESSION['privileges']) && in_array("/covid-19/requests/export-covid19-requests.php", $_SESSION['privileges'])) { ?>
								...
								<button style=" margin: 0px 5px; " class="btn btn-primary btn-sm pull-right"
									...
									onclick="hideAdvanceSearch('filter','advanceFilter');"><span>
										<?= _("Show Advanced Search Options"); ?>
									</span></button>
							</td>
						</tr>
					</table>
