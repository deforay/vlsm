...
$bQuery="SELECT vl.sample_code,vl.sample_batch_id,vl.eid_id,vl.facility_id,vl.result,vl.result_status,f.facility_name,f.facility_code FROM eid_form as vl INNER JOIN facility_details as f ON vl.facility_id=f.facility_id WHERE  (vl.is_sample_rejected IS NULL OR vl.is_sample_rejected = '' OR vl.is_sample_rejected = 'no') AND (vl.sample_rejection_reason IS NULL OR vl.sample_rejection_reason ='' OR vl.sample_rejection_reason = 0) AND vlsm_country_id = '".$configResult[0]['value']."' AND vl.sample_code!='' AND vl.sample_batch_id = $id ORDER BY vl.last_modified_datetime ASC";
//error_log($bQuery);die;
$batchResultresult = $db->rawQuery($bQuery);

$query="SELECT vl.sample_code,vl.sample_batch_id,vl.eid_id,vl.facility_id,vl.result,vl.result_status,f.facility_name,f.facility_code FROM eid_form as vl INNER JOIN facility_details as f ON vl.facility_id=f.facility_id WHERE (vl.sample_batch_id IS NULL OR vl.sample_batch_id = '') AND (vl.is_sample_rejected IS NULL OR vl.is_sample_rejected = '' OR vl.is_sample_rejected = 'no') AND (vl.sample_rejection_reason IS NULL OR vl.sample_rejection_reason ='' OR vl.sample_rejection_reason = 0) AND (vl.result is NULL or vl.result = '') AND vlsm_country_id = '".$configResult[0]['value']."' AND vl.sample_code!='' ORDER BY vl.last_modified_datetime ASC";
//error_log($query);die;
