...
?>
      ...
      <div class="row">
	    ...
	    <span style="display: none;position:absolute;z-index: 9999 !important;color:#000;padding:5px;margin-left: 325px;" id="showhide" class="">
	      ...
	      <?php if(isset($_SESSION['privileges']) && in_array("eid-add-batch.php", $_SESSION['privileges'])){ ?>
              <a href="eid-add-batch.php" class="btn btn-primary pull-right"> <i class="fa fa-plus"></i> Create New Batch</a>
	      <?php } ?>
    ...
    $.blockUI();
        ...
        <?php } ?>
            ...
            "fnServerData": function ( sSource, aoData, fnCallback ) {
              aoData.push({"name": "type", "value": "eid"});
              $.ajax({
                  "dataType": 'json',
                  "type": "POST",
                  "url": sSource,
  ...
  function generateBarcode(bId){
    $.post("/batch/generateBarcode.php",{id:bId,type:'eid'},
      function(data){
  ...
  function deleteBatchCode(bId,batchCode)
  {
    ...
    if(conf){
      $.post("/batch/deleteBatchCode.php",{id:bId, type: 'eid'},
      function(data){
        if(data==1){
          alert("Batch deleted");
        }else{
