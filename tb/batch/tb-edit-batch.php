...
$bQuery = "SELECT vl.sample_code,vl.sample_batch_id,vl.tb_id,vl.facility_id,vl.result,vl.result_status,f.facility_name,f.facility_code FROM form_tb as vl INNER JOIN facility_details as f ON vl.facility_id=f.facility_id WHERE  (vl.is_sample_rejected IS NULL OR vl.is_sample_rejected = '' OR vl.is_sample_rejected = 'no') AND (vl.reason_for_sample_rejection IS NULL OR vl.reason_for_sample_rejection ='' OR vl.reason_for_sample_rejection = 0) AND vl.sample_code!='' AND vl.sample_batch_id = ? ORDER BY vl.last_modified_datetime ASC";
//error_log($bQuery);die;
$batchResultresult = $db->rawQuery($bQuery, array($id));

$query = "SELECT vl.sample_code,vl.sample_batch_id,vl.tb_id,vl.facility_id,vl.result,vl.result_status,f.facility_name,f.facility_code FROM form_tb as vl INNER JOIN facility_details as f ON vl.facility_id=f.facility_id WHERE (vl.sample_batch_id IS NULL OR vl.sample_batch_id = '') AND (vl.is_sample_rejected IS NULL OR vl.is_sample_rejected = '' OR vl.is_sample_rejected = 'no') AND (vl.reason_for_sample_rejection IS NULL OR vl.reason_for_sample_rejection ='' OR vl.reason_for_sample_rejection = 0) AND (vl.result is NULL or vl.result = '') AND vl.sample_code!='' AND vl.vlsm_country_id = ? ORDER BY vl.last_modified_datetime ASC";
//error_log($query);die;
...
<div class="content-wrapper">
	...
	<section class="content">
		<div class="box box-default">
			...
			<div class="box-body">
				<!-- form start -->
				<form class="form-horizontal" method='post' name='editBatchForm' id='editBatchForm' autocomplete="off" action="tb-edit-batch-helper.php">
					<div class="box-body">
						<div class="row">
							<div class="col-md-6">
							...
							<div class="col-md-6"><a href="tb-edit-batch-position.php?id=<?php echo base64_encode($batchInfo[0]['batch_id']); ?>" class="btn btn-default btn-xs" style="margin-right: 2px;margin-top:6px;" title="Edit Position"><i class="fa fa-sort-numeric-desc"> Edit Position</i></a></div>
						</div>
...
<script type="text/javascript">
	...
	$(document).ready(function() {
		...
		?>
					...
					<?php } ?>
					resultSampleArray.push('<?php echo $sample['form_tb']; ?>');
		<?php $r++;
				}
			}
		}
