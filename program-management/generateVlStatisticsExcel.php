...
//Sort Facilities
if(isset($_POST['lab']) && count($_POST['lab'])> 0){
    $lab = implode(',',$_POST['lab']);
    $facilityQuery ="SELECT f.facility_name from facility_details as f where f.facility_id IN ($lab)";
    $facilityResult = $db->query($facilityQuery);
    $facilities = array();
    foreach($facilityResult as $facility){
        $facilities[] = ucwords($facility['facility_name']);
    }
}
if(isset($_SESSION['vlStatisticsQuery']) && trim($_SESSION['vlStatisticsQuery'])!=""){
 $sResult = $db->rawQuery($_SESSION['vlStatisticsQuery']);
 $excel = new PHPExcel();
 ...
 $sheet->setCellValue('G1', html_entity_decode((count($facilityResult) >0)?implode(',',$facilities):'', ENT_QUOTES, 'UTF-8'), \PHPExcel_Cell_DataType::TYPE_STRING);
