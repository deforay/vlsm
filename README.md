...
InteLIS (formerly VLSM) is a simple, open source Sample Management System for tracking samples for HIV Viral Load, EID, Covid-19, TB, Hepatitis and others.
...
### Ubuntu Setup
If you are using Ubuntu 24.04 (recommended), then you can find setup instructions here : [Ubuntu Setup](docs/Ubuntu-Setup.md)

#### How do I get started?
* Download the Source Code and put it into your server's root folder (www or htdocs).
* Run `composer update`. For e.g. you might have placed the code in `/var/www/intelis` path. Then you would run `composer update` inside `/var/www/intelis` folder.
* Open phpMyAdmin or any MySQL client and create a blank database called `intelis`
...
* Next we create a virtual host pointing to the root folder of the source code. You can see an example below (assuming the full path to VLSM is `/var/www/intelis`) :
...
* Once you have the application set up, you can visit the InteLIS virtualhost URL http://intelis.example.org/ and set up the admin user.
